...
function initCategoriesSelector() {
  ...
  function updateRenderer() {
    ...
    for (const { label, id } of catLis) {
		...
		content +=
			'<span class="cat-container inline-flex items-center px-2 py-1 mr-2 text-sm font-medium text-' + color + '-800 bg-' + color + '-100 rounded dark:bg-' + color + '-900 dark:text-' + color + '-300" data-cat-id="' +
			id +
			'">\
				' +
			label +
			'\
				<button type="button" class="inline-flex items-center p-0.5 ml-2 text-sm text-' + color + '-400 bg-transparent rounded-sm hover:bg-' + color + '-200 hover:text-' + color + '-900 dark:hover:bg-' + color + '-800 dark:hover:text-' + color + '-300" aria-label="Remove">\
